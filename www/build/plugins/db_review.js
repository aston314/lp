    ...
    function douban_review(kpid, imdbid, num) {
        if (kpid != '') {
            $.get('https://movie.douban.com/j/subject_suggest?q=' + imdbid, function (data) {
                console.log(data[0])
                // 
                network["native"]('https://m.douban.com/rexxar/api/v2/movie/' + data[0].id + '/interests?count=20&order_by=hot&anony=0&start=0&ck=&for_mobile=1', function (json) {
                    // var button = '';
                    // json.interests.forEach(function (element) {
                    //     button = button + '<div class="items-line__head" style="margin-bottom: 0.4em;"><div class="items-line__title">' + element.create_time + '</div><div>作者: ' + element.user.name + '<br />' + element.rating.value + '分</div></div><div class="items-line__body"><div class="full-descr"><div class="full-descr__left"><div>' + element.comment + '</div></div></div></div>';
                    // });
