(function () {
  ...
  function component(object) {
    ...
    this.convertToShortDate = function (dateTimeString) {
      const datePart = dateTimeString.substr(0, 10);
      return datePart;
    }
          ...
          if (datatype) {
            datatype.forEach(function (item, index) {
              ...
              listlink.data[0].media.push({
                ...
                max_quality: item.type == 1 ? this_2.convertToShortDate(item.modified) + ' / ' + '文件夹' : this_2.convertToShortDate(item.modified) + ' / ' + item.name.substr(item.name.lastIndexOf('.') + 1).toUpperCase() + ' / ' + get_size(item.size),
