(function () {
    ...
    function LGDY(object) {
        ...
        this.next = function () {
            ...
            //if (object.gotopage) {
                ...
                network.silent(object.url + 'index_' + object.page + '.html', function (str) {
        ...
        this.card = function (str) {
            ...
            // };
            //console.log(page)
            $('#post_container a.zoom', str).each(function (i, html) {
                //if ($('.tgme_widget_message_text.js-message_text', html).text().match(/https:\/\/www\.aliyundrive\.com\/s\/([a-zA-Z\d]+)/)) {
                    card.push({
                        ...
                        title: html.title.indexOf(' ') !== -1 ? html.title.substring(0, html.title.indexOf(' ')) : html.title,
                        original_title: '',
                        title_org: '',
                ...
                card.on('hover:enter', function (target, card_data) {
                    ...
                    network.silent(element.url, function (str) {
                        //$('.btn-group a.line-pay-btn', str).each(function (i, str) {
                            $('a[href*="magnet:?xt=urn:btih:"]', str).each(function (i, html) {
                            sources.push({
                                title:  html.text,
                                url: html.href,
                            });
            ...
            info.find('.open--find').on('hover:enter hover:click', function () {
                Lampa.Input.edit({
                    ...
                    if (new_value) {
                        ...
                        var search_tempalte = 'http://www.languangdy.com/e/search/index.php#msearchword';
                        var searchurl = search_tempalte.replace('#msearchword',encodeURIComponent(new_value));
                        Lampa.Activity.push({
                            //	url: cors + a.url,
                            url: searchurl,
                            title: '蓝光电影网 - 搜索"' + new_value + '"',
                            component: 'lgdy',
                            page: 1
                        });
                    }
    ...
    {
        title: 'Bluray蓝光原盘',
        url: 'http://www.languangdy.com/Bluray/'
    },
    {
        title: '蓝光1080P',
        url: 'http://www.languangdy.com/1080P/'
    },
    {
        title: '4K超高清',
        url: 'http://www.languangdy.com/4K/'
    },
    {
        title: '其他资源',
        url: 'http://www.languangdy.com/other/'
    }];

    function startLGDY() {
        window.plugin_lgdy_ready = true;
        Lampa.Component.add('lgdy', LGDY);

        function addSettingsLGDY() {
            ...
            var menu_item = $('<li class="menu__item selector focus" data-action="channel"><div class="menu__ico">' + ico + '</div><div class="menu__text">蓝光</div></li>');
        ...
        if (window.appready) addSettingsLGDY()
        else {
            Lampa.Listener.follow('app', function (e) {
                if (e.type == 'ready') addSettingsLGDY()
            })
        }
    ...
    if (!window.plugin_lgdy_ready) startLGDY();
