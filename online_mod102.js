...
(function () {
        ...
        $(h).each(function (i, html) {
          ...
          if ($(html).text().match(/https:\/\/www\.aliyundrive\.com\/s\/([a-zA-Z\d]+)/)) {
            rslt.push({
              ...
              quality: '发布于：' + formatDate(new Date(($('.time', html).attr('datetime')))),
              //quality: $('p',html).text().replace(/文件夹/,'目录'),
              title: $('.highlight:first', html).text() + ($(html).html().match(/<\/mark>(.+?)<br>/) ? $(html).html().match(/<\/mark>(.+?)<br>/)[1].replace(/<b>|<\/b>/g, '').replace(/<mark/g, '<span').replace(/mark>/g, 'span>').replace('》', '') : ''),
              season: '',
              episode: '',
              info: ''
            });
    ...
    function component(object) {
      ...
      var sources = {
        ...
        阿里云盘发布频道: new tg_share_channel(this, object, 'Aliyundrive_Share_Channel'),
        阿里云盘盘: new tg_share_channel(this, object, 'yunpanpan'),
        肯德基电影院: new tg_share_channel(this, object, 'XiangxiuNB'),
      };
      ...
      var filter_sources = ['酷云',  '阿里云盘', '小纸条', '在花盘', '阿里云盘发布频道', '阿里云盘盘', '肯德基电影院', 'videocdn', 'rezka', 'kinobase', 'collaps', 'cdnmovies', 'filmix', 'hdvb', 'videoapi']; // шаловливые ручки
      ...
      this.destroy = function () {
        ...
        sources.阿里云盘盘.destroy();
        sources.肯德基电影院.destroy();
        doregjson.rule.forEach(function (elem) {
