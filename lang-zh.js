Lampa.Lang.addCodes({zh:'简体中文'});

Lampa.Lang.AddTranslation('zh',{
    ...
    settings_rest_tmdb_prox: '代理TMDB',
    settings_rest_tmdb_posters: 'TMDB海报的分辨率',
    settings_rest_screensaver: '屏保',
    ...
    settings_rest_device_placeholder: 'For例如：我的Lampa',
    ...
    settings_player_reset_descr: '重置应用程序中选中的Android播放器',
    settings_player_path: '播放器路径',
    settings_player_path_descr: '指定播放器.exe的路径',
    settings_player_normalization: '声音归一化',
    settings_player_normalization_descr: '将声音归一化水平，降低响亮的声音并提高安静的声音。',
    ...
    settings_player_subs_stroke_use_descr: '为了更好的可读性，字幕将用黑色勾勒出来',
    ...
    settings_plugins_add_descr: '要删除已添加的插件，按住或双击(OK) 键',
    ...
    settings_server_link_two_descr: '提供额外链接到 TorrServer 脚本',
    ...
    torent_nohash_do_five: '指定哪部电影，哪个发行版，如果可能的话，还有这张发行版的照片',
    ...
    torrent_error_info_1: '确保你已经在安装了 TorrServe 的设备上启动了。',
    torrent_error_info_2: '一个常见的错误，带有 TorrServe 的设备的 IP 地址已经改变。确保 IP 地址您输入的 - {ip} - 与安装了 TorrServe 的设备的地址匹配。',
    torrent_error_info_3: '要连接到 TorrServe，您必须在开头指定协议 http://，在地址末尾指定端口：8090 .确保IP地址后面有一个端口，你当前的地址是{ip}',
    torrent_error_info_4: '经常出现，杀毒或防火墙可以通过IP地址阻止访问，尝试禁用杀毒和防火墙。',
    torrent_error_info_5: '在任何其他设备上在同一网络上，在浏览器中打开 {ip} 地址，检查 TorrServe Web 界面是否可用。',
    torrent_error_info_6: '如果在所有检查后仍然出现连接错误，请尝试重新启动 TorrServe 和 Internet 适配器。',
    torrent_error_info_7: '如果问题仍然存在，请写信到 Telegram 群组@lampa_group，并附上文字（Lampa 未连接到 TorrServe 经所有检查，当前地址为 {ip}）',
    ...
    torrent_install_text: 'TorrServe 是一个允许您在线查看 torrent 文件内容的应用程序。<br><br>有关安装的更多详细信息，请参阅下面的电报组。',
    ...
    torrent_parser_label_descr: '标记a拿着旗子的手（查看）',
    ...
    torrent_parser_magnet_error: '失败获取磁力链接',
    torrent_parser_torlook_fallback_search_notification: 'Jackett 解析器不可用或连接设置不正确。搜索 Torlook...',
    about_text: '该应用程序完全免费，并使用公共链接获取有关视频、新版本、热门电影等的信息。使用所有可用信息仅用于教育目的，该应用程序不使用自己的服务器来分发信息。',
    about_channel: '我们的频道',
    about_group: '组',
    about_version: '版本',
    about_donate: '捐赠',
    ...
    title_collections_ivi: 'ivi 收藏',
    title_collections_okko: 'okko 收藏',
    title_company: '公司',
    title_actors: '演员',
    title_actor: '演员',
    title_actress: '女演员',
    ...
    plugins_catalog_popular: '用户中的流行插件',
    plugins_catalog_popular_descr: '从未知来源安装可能会导致应用程序无法正常工作。 ',
    plugins_online: '在线查看',
    plugins_check_fail: '未能测试插件的功能。但是，这并不意味着插件不起作用。重新加载应用程序，看看插件是否正在加载。',
    ...
    plugins_ok_for_check: '点击（确定）测试插件',
    ...
    settings_param_player_timecode_ask: '要问',
    ...
    helper_torrents_view: '按住（确定）键重置时间码并显示菜单',
    fav_sync_title: '书签同步',
    fav_sync_text: '您最喜欢的书签与您连接。连接同步并查看任何<br><br>为此，请在网站 www.cub.watch 上注册，创建配置文件并登录到应用程序。',
    fav_sync_site: '网站',
    fav_remove_title: '从历史记录中删除',
    fav_remove_descr: '删除已选择卡片',
    ...
    notice_in_quality: '如',
    copy_link: '复制视频链接',
    copy_secuses: '复制到剪贴板的链接',
    copy_error: '复制链接时出错',
    account_sync_to_profile: '所有书签将移动到个人资料',
    ...
    settings_player_hls_descr: '如果你不知道为什么，请不要碰这个参数。',
    notice_none_account: '你没有通知，收藏该系列并等待新剧集的通知。',
    ...
    player_normalization_power_title: '归一化功率',
    player_normalization_smooth_title: '归一化速度',
    player_normalization_step_low: '低',
    ...
    extensions_set_url: '输入插件 url',
    extensions_ready: '此插件已安装',
    extensions_no_info: '没有信息',
    ...
    extensions_from_popular: '热门插件',
    extensions_from_lib: '插件库',
    extensions_from_connected: '已连接插件',
    title_choice_language: '选择语言',
    search_nofound: '根据您的要求没有找到任何内容。',
    ...
    settings_cub_sync_quality: '质量同步',
    settings_cub_sync_search: '搜索历史同步',
    settings_cub_sync_recomends: '推荐同步',
    title_unsubscribe: '退订',
    subscribe_error: '订阅时出错，请稍后再试',
    subscribe_noinfo: '检索信息失败，请稍后再试',
    account_create: '使用 CUB 帐户发现更多机会。在 <span class="account-modal__site">www.cub.watch</span> 注册并获得同步书签的访问权限, 时间码和其他 CUB 帐户功能。',
    account_premium: '使用 CUB 高级帐户发现更多功能。增加限制和访问服务的其他功能。',
    account_premium_more: '了解有关 CUB 高级的更多信息',
    account_limited: '您已达到最大值限制。使用 CUB 高级帐户提高限制。在 <span class="account-modal__site">www.cub.watch/premium</span> 了解更多信息',
    settings_rest_tmdb_prox_auto: '自动启用代理',
    settings_rest_card_quality: '质量标记',
    settings_rest_card_quality_descr: '在卡片上显示质量标记',
    settings_rest_card_episodes: '情节标记',
