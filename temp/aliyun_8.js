...
/*            console.log("这里");
    ...
    Lampa.Settings.listener.follow('open', function (e) {
        if (e.name == 'mod_aliyun') {
          e.body.find('[data-name="aliyun_qr"]').unbind('hover:enter').on('hover:enter', function () {
            var modal = $('<script src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js"></script><div><div class="broadcast__text">扫码获取 Refresh token</div><div class="broadcast__device selector" style="text-align: center;"><div id="qrcode-container"  style="display: flex; justify-content: center; align-items: center;"></div></div><div class="broadcast__scan"><div></div></div></div></div>');
            Lampa.Modal.open({
              title: '',
              html: modal,
            ...
            ping_auth = setTimeout(function () {
              ...
              network.quiet('https://easy-token.cooluc.com/qr', function (found) {
                if (found.success) {
                  c = found.data;
                  //modal.find('.qrcode-img').attr( "src", found.codeContent ); //modal.find('.broadcast__scan').remove()
                  var qrc = new QRCode(
                    document.getElementById("qrcode-container"),
                    found.data.codeContent
                  );
    ...
    function getcode() {
        network.clear();
        network.timeout(10000);
        network.quiet("https://easy-token.cooluc.com/ck", function (found) {
            if (found.pds_login_result == 'fail') {

            } else {
                clearInterval(i);
                Lampa.Storage.set("aliyun_token", found.pds_login_result.refreshToken);
                Lampa.Modal.close();
                Lampa.Controller.toggle('settings_component');
            }
