...
(function () {
    ...
    var extract_rule = {
      ...
      },
      {
        ...
        link_selector: '',
        link_folder: 'play',
        detail_url_selector: '.play-content',
        use_referer: true,
        js_execute_key: ['maccms', 'player_aaaa']
      },
      {
      ...
      //   scVodName : '',
      //   scVodId : '',
      //   search_list_reg : '<ul class="tbox_m">([\\s\\S]*?)<\/ul>',
      //   search_list_have_string : 'vbox',
      //   search_list_find_reg : '<li class="vbox">(.*?)<\/li>',
      //   title_selector : '.vbox_t item-lazy', 
      //   title_selector_value : 'title',
      //   link_selector : 'href',
      //   link_folder : 'play',
      // //   detail_url_reg : '<div class="stui-pannel_bd col-pd clearfix">(.*?)<\/div>',
      //   detail_url_selector : 'div:nth-child(9) > div.tabs_block2',
      ...
      //   link_selector : '',
      //   link_folder : 'vodplay',
      //   detail_url_reg : '<div class="module-list sort-list tab-list play-tab-list active" id="panel2">(.*?)<\/div>',
      //   detail_url_selector : '.stui-pannel_bd.col-pd.clearfix',
      ...
      // },
      {
        ...
        },
        {
          ...
          link_selector: '',
          link_folder: 'play',
          detail_url_selector: 'div.module-play-list-content.module-play-list-base',
          use_referer: true,
          js_execute_key: ['maccms', 'player_aaaa']
        },
        {
          ...
          scVodName: '',
          scVodId: '',
          search_list_reg: '<div class="bt_img mi_ne_kd search_list">[\\s\\S]*?(<ul>[\\s\\S]*?<\/ul>)[\\s\\S]*?<\/div>',
          search_list_have_string: 'dytit',
          search_list_find_reg: '<li>([\\s\\S]*?)<\/li>',
          title_selector: '.dytit',
          title_selector_value: 'text',
          link_selector: '.dytit a',
          link_folder: '',
    ...
    function web(component, _object, rule) {
      ...
      // function getRemote(remote_url, type, datatype, data) {
      //   return $.ajax({
      //     type: type,
      //     url: remote_url,
      //     dataType: datatype,
      ...
      $.each(math, function (i, a) {
          ...
          if (doreg.search_json) {
              ...
              item.Link = $(doreg.link_selector, element).attr('href').indexOf('http') ==-1 ?doreg.websitelink + $(doreg.link_selector, element).attr('href'):$(doreg.link_selector, element).attr('href');
          };
          ...
          network["native"](item.Link, function (str) {
              ...
              var math = $(doreg.detail_url_selector, str.replace(/\n|\r/g, ''));
      ...
      function doparse(element, view, url1_, url, data) {
        ...
        while (match = re.exec(str)) {
          var cc = match[1].indexOf('./') !== -1 ? match[1].replace('./', MacPlayer_.split(url1_[0])[0] + '/' + url1_[1] + '/') : match[1];
          if (!/jquery|setting|player|c606e5caeee702a784a0204d31ea3403|35a898211164a6b8a9a21a045dba9f8a|805d73dedddd5daf87bdbd38488362f8|33d6112475ac4d264c333fe9a5252aff|hls|flv/.test(cc)) {
            aa.push(cc);
          }
            ...
            if (element.file) {
              network.silent(element.file, function (str) {
                if (str) {
                  ...
                  var czspp = str.match(/window.wp_nonce=".*";([\s\S]*?)\/\//);
    ...
    function kunyu77(component, _object, rule) {
      ...
      this.search = function (_object, kinopoisk_id) {
        ...
        var url;
        kinopoisk_id === parseInt(kinopoisk_id, 10) ? url='http://api.kunyu77.com/api.php/provide/videoPlaylist?ids='+kinopoisk_id: url= 'http://api.kunyu77.com/api.php/provide/searchVideo?searchName='+encodeURIComponent(object.movie.title);
                  ...
                  if (json) {
                      ...
                      if (json.data.length !== 0) {
                          ...
                          if (json.data.length == 1) {
                              ...
                              url = 'http://api.kunyu77.com/api.php/provide/videoPlaylist?ids=' + id;
                              network.silent(url, function (json) {
                                  if (json) {
                                      if (json.data.episodes.length == 0) {
                                          component.emptyForQuery(select_title)
    ...
    function component(object) {
      ...
      if (filter_sources.indexOf(balanser) == -1) {
        balanser = '阿里云盘';
        Lampa.Storage.set('online_mod_balanser', '阿里云盘');
      }
