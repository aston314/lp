//18.07.2022 - Add belarusian translation
...
(function () {
      ...
      results.slice(0, 1).forEach(function (movie) {
        if (movie.episodes) {
          movie.episodes.forEach(function (episode) {
            if (episode.season_num == choice.season + 1) {
              episode.media.forEach(function (media) {
                if (filter_items.voice_info[choice.voice] && media.translation.id == filter_items.voice_info[choice.voice].id) {
                  filtred.push({
  ...
  function kinobase(component, _object) {
    ...
    this.search = function (_object, kp_id, sim) {
      ...
      if (this.wait_similars && sim && sim[0].is_similars) return getPage(sim[0].link);
      object = _object;
        ...
        item.on('hover:enter', function () {
          ...
          if (element.stream) {
            ...
            var first = {
              url: element.stream,
              timeline: view,
              title: element.season ? element.title : element.voice ? object.movie.title + ' / ' + element.voice : element.title,
              subtitles: element.subtitles,
              quality: element.qualitys
            };
  ...
  function hdvb(component, _object) {
    ...
    this.search = function (_object, _kinopoisk_id) {
      object = _object;

      if (isNaN(_kinopoisk_id)) {
        ...
        if (translations.serial == 1) {
          ...
          for (var keys in translations.playlists) {
            ...
            for (var keye in seasons) {
              ...
              var qualities = Object.keys(episode); //if (qualities) qualities = qualities.filter( function (elem) { return parseInt(elem) <= parseInt(????) && parseInt(elem) !== 0 });

              var qualitie = Math.max.apply(null, qualities);
              var link = episode[qualitie];
              folder[keye] = {
                id: keys + '_' + keye,
                comment: keye + ' ' + Lampa.Lang.translate('torrent_serial_episode') + '<br><i>' + qualitie + '</i>',
                file: link,
                episode: keye,
                season: keys,
                quality: qualitie,
    ...
    function getFile(element, max_quality) {
      ...
      if (element.season) {
        ...
        qualities = extract[element.translation].qualities; //let filter_data = Lampa.Storage.get('online_mod_filter', '{}');
      ...
      if (file) {
        ...
        for (var n in qualities) {
          if (parseInt(qualities[n]) <= select_quality && qualities.length > 1) {
      ...
      return {
        file: file_filtred.length > 1 ? file_filtred : file,
        quality: quality
      };
    }
    /**
    ...
    function filter() {
      ...
      results.forEach(function (translation, keyt) {
        if (translation.serial == 0) ; else if (translation.serial == 1) {
    ...
    function getStream(element, call, error) {
      ...
      if (element.link.substr(-5) === ".m3u8") return call(getFile(element, element.quality));
      network.clear();
      ...
      var url = component.proxy('hdvb') + 'http://freebie.tom.ru/hdvburl?v=600&id_kp=' + kinopoisk_id;
      ...
      network.silent(url, function (str) {
        ...
        if (result.serial == 1) {
          if (result.key) {
            result.playlists[element.season][element.episode][result.max_qual] = str; //result.playlists[ element.season ][ element.episode ] = {};
          ...
          result.playlists[result.max_qual] = str; //result.playlists = {};
        ...
        if (results) {
          ...
          network.silent(element.link, function (plist) {
            ...
            ['2160', '1080', '720', '480', '360'].forEach(function (elem) {
              if (plist.indexOf('/' + elem + '/') > 0) if (results[element.translation].serial == 1) results[element.translation].playlists[element.season][element.episode][elem] = element.link;else results[element.translation].playlists[elem] = element.link;
            });
            extractData();
            append(filtred());
            call(getFile(element, element.quality));
  ...
  function web(component, _object, rule) {
    ...
    function append(items) {
      ...
      var viewed = Lampa.Storage.cache('online_view', 5000, []);
      items.forEach(function (element) {
        var hash = Lampa.Utils.hash(element.quality ? [element.quality, object.movie.original_title].join('') : object.movie.original_title);
        var view = Lampa.Timeline.view(hash);
        var item = Lampa.Template.get('online_mod', element);
        var hash_file = Lampa.Utils.hash(element.quality ? [element.quality, object.movie.original_title, element.title].join('') : object.movie.original_title + 'libio');
        element.timeline = view;
        item.append(Lampa.Timeline.render(view));
  ...
  function component(object) {
    ...
    var filter_sources = [ 'videocdn', 'rezka', 'kinobase', 'collaps', 'cdnmovies', 'filmix', 'hdvb']; // шаловливые ручки
    filter_sources = sname.concat(filter_sources);
    
    if (filter_sources.indexOf(balanser) == -1) {
      balanser = 'libvio';
      Lampa.Storage.set('online_mod_balanser', 'libvio');
    }
    ...
    this.find = function () {
      ...
      var arr = ['videocdn', 'rezka', 'kinobase', 'collaps', 'cdnmovies', 'filmix', 'hdvb'];
      if (arr.indexOf(balanser) === -1) {
        ...
        var url = prox ? prox + 'https://videocdn.tv/api/short' : 'http://cdn.svetacdn.in/api/short';
        ...
        var alt_search = function alt_search() {
          network.timeout(1000 * 15);
          network.silent(_this2.proxy('kp') + 'https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-keyword?keyword=' + encodeURIComponent(_this2.cleanTitle(query)), function (json) {
        ...
        var pillow = function pillow(a, c) {
          if (balanser !== 'videocdn') alt_search(); else _this2.empty(network.errorDecode(a, c));
        };
        ...
        var letgo = function letgo(imdb_id) {
          ...
          network.silent(url_end, function (json) {
            if (json.data && json.data.length) display(json); else {
              network.silent(Lampa.Utils.addUrlComponent(url, 'title=' + encodeURIComponent(query)), function (json) {
                if (balanser === 'videocdn' || json.data && json.data.length) display(json); else alt_search();
              }, pillow.bind(_this2));
            }
          }, pillow.bind(_this2));
        };
