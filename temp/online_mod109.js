    ...
    function web(component, _object, rule) {
      ...
      this.search = function (_object, kinopoisk_id) {
        ...
        network.silent(url1, function (str) {
          ...
          var searchresult = doreg.search_json ? (parsedData[doreg.node_json] ? parsedData[doreg.node_json].length : 0) : $(doreg.search_html_selector, parsedData).find('a').length;
      ...
      function filtred() {
        var filter_data = Lampa.Storage.get('online_mod_filter', '{}');
        return component.order[filter_data.order] ? (component.order[filter_data.order].id == 'invers' ? rslt.reverse() : rslt) : rslt;
        //return rslt;
      }
      /**
       * Показать файлы
