      ...
      $.each(math, function (i, a) {
          ...
          network["native"](item.Link, function (str) {
              ...
              $(math).find('a').each(function (i, a) {
                  rslt.push({
                      file: doreg.listlink ? doreg.websitelink + $(a).attr('href') : matches[0].attr('href'),
                            ...
                            var file = file_[0];
                                  if (MacPlayer_ && MacPlayer.PlayUrl.indexOf('.m3u8') == -1) {
                                    if (!doreg.use_referer) {
                                      ...
                                      //console.log(MacPlayer_.split(url1_[0])[0]+'/'+url1_[1]+'/')
                                      network.silent(MacPlayer_, function (str) {
                                        ...
                                        while (match = re.exec(str)) {
                                          ...
                                          if (!/jquery|setting|player|hls|flv/.test(cc)) {
                                            aa.push(cc);
                                          }
                                        ...
                                        $.getMultiScripts(aa, '').done(function () {
                                          ...
                                          //var parse_url = "urls = '(.+?)'"
                                          //var mathurl = new RegExp(parse_url, 'g').exec(str);
                                                ...
                                                var url1_ = MacPlayer_.replace('/' + doreg.link_folder + '/','').match(/\/([^\/]+)\/[^\/]+$/);
                                                  ...
                                                  $.getMultiScripts(aa, '').done(function () {
                                                    ...
                                                    if (setting_js) {
                                                      ...
                                                      var e_js = s_js.match(/("sign"|sign): ["'](.+?)["']/)[2];
                                                      //console.log(e_js)
                                                      //console.log(config.url,config.vkey,config.token,e_js);
                                                      $.ajax({
                                                        url: d_sj,
                                                        type: b_js,
                                                        dataType: 'JSON',
