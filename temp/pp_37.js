    ...
    this.create = function () {
        ...
        if (info.loginInfo) {
          var getlink = object.url;
            url =  PikPakProxy() + "https://api-drive.mypikpak.com/drive/v1/files?parent_id=" + getlink + "&thumbnail_size=SIZE_LARGE&with_audit=true&limit=100&filters=%7B%22phase%22:%7B%22eq%22:%22PHASE_TYPE_COMPLETE%22%7D,%22trashed%22:%7B%22eq%22:false%7D%7D";
            // PikPakProxy() + 
            // jsonsearch = {
            //   parent_id:getlink,
        ...
        items.forEach(function (element) {
          ...
          item.on('hover:focus', function (e) {
            ...
            if (element.type == 'drive#folder') {
              //setTimeout(function () {
              var reg = /[\u4e00-\u9fa5|\u3002|\uff1f|\uff01|\uff0c|\u3001|\uff1b|\uff1a|\u201c|\u201d|\u2018|\u2019|\uff08|\uff09|\u300a|\u300b|\u3008|\u3009|\u3010|\u3011|\u300e|\u300f|\u300c|\u300d|\ufe43|\ufe44|\u3014|\u3015|\u2026|\u2014|\uff5e|\ufe4f|\uffe5\d+|\/]+/;
              //var reg = /^(.+?)[.( \t]*(?:(?:(19\d{2}|20(?:0\d|1[0-9]))).*|(?:(?=bluray|\d+p|brrip|WEBRip)..*)?$)/gmi;
              ...
              //   dataType: 'text'
              // });
              network.silent('https://api.safone.tech/tmdb?limit=1&query=' + encodeURIComponent(chinese_title), function (json) {
                if (json.results.length >0) {
                  //$(".files__title").append("<br/> <br/> 豆瓣："+rating_douban);
                  $(".full-start__poster").after('<div class="card__type" style="left:1em;top:70">TMDB：' + json.results[0].rating.toFixed(1) + '</div>');
                  //$(".full-start__img").after('<div class="card--new_ser" style="right: -0.6em;position: absolute;background: #168FDF;color: #fff;top: 0.8em;padding: 0.4em 0.4em;font-size: 1.2em;-webkit-border-radius: 0.3em;-moz-border-radius: 0.3em;border-radius: 0.3em;">豆瓣：'+ rating_douban +'</div>');
                
                if (json.results[0].poster) {
                  $(".full-start__img").attr('src', 'https://dsag3w1du2cu2.cloudfront.net/' + json.results[0].poster.replace('original','w200'));
                } else {
                  $(".full-start__img").attr('src', './img/img_broken.svg');
                };
               }else{
            ...
            if (element.type == 'drive#folder') {
              ...
              network.silent(PikPakProxy() + 'https://api-drive.mypikpak.com/drive/v1/files/' + myurl + '?_magic=2021&thumbnail_size=SIZE_LARGE', function (json) {
                if (json) {
                  //_this4.start();
                  var file = json.medias[0].link.url;
