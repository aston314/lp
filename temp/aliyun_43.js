              ...
              if (json) {
                if (getShareId === null) {
                  ...
                  if (object.movie.img == './img/img_broken.svg') {
                    ...
                    //   dataType: 'json'
                    // });
                    var chinese_title = object.movie.title.replace(/《|【|》|】|\./g, ' ').match(reg) ? object.movie.title.replace(/《|【|》|】|\./g, ' ').match(reg)[0] : object.movie.title;
                    network.silent('https://m.douban.com/search/?query=' + encodeURIComponent(chinese_title), function (json) {
                  ...
                  if (json) {
                    ...
                    if (file_id_json.code) {
                      ...
                      if (object.movie.img == './img/img_broken.svg') {
                        ...
                        network.silent('https://m.douban.com/search/?query=' + encodeURIComponent((file_id_json.file_infos[0].file_name.match(reg)[0] ? file_id_json.file_infos[0].file_name.match(reg)[0] : file_id_json.file_infos[0]).replace(/《|【|》|】|\./g, ' ').match(reg)[0]), function (json) {
                          var poster_douban = $('.search-results-modules-name:contains(电影) + ul > li:nth-child(1) > a > img', json).attr('src');
                          //var rating_douban = $('.search-results-modules-name:contains(电影) + ul > li:nth-child(1) > a > div > p > span:nth-child(2)', json).text();
                          // if (rating_douban) {
                          //   $(".files__title").append("<br/> <br/> 豆瓣：" + rating_douban);
            ...
            item.on('hover:focus', function (e) {
              ...
              if (element.type == 'folder') {
                setTimeout(function () {
                  ...
                  $('.card__type').remove();
                  var chinese_title = element.title.replace(/《|【|》|】|\./g, ' ').match(reg) ? element.title.replace(/《|【|》|】|\./g, ' ').match(reg)[0] : element.title;
                  network.silent('http://proxy.cub.watch/cdn/https://m.douban.com/search/?query=' + encodeURIComponent(chinese_title), function (json) {
                    var poster_douban = $('.search-results-modules-name:contains(电影) + ul > li:nth-child(1) > a > img', json).attr('src');
                    var rating_douban = $('.search-results-modules-name:contains(电影) + ul > li:nth-child(1) > a > div > p > span:nth-child(2)', json).text();
                    if (rating_douban){
                      //$(".files__title").append("<br/> <br/> 豆瓣："+rating_douban);
