(function () {
        ...
        $.ajax({
            ...
            success: function success(j) {
                if (j.length > 0) {
                    ...
                    component.empty('哦: Alist配置无法加载，请检查JSON地址。' );
                }
            },
  ...
  function component(object) {
          ...
          var getlink = object.url;
            ...
            $.ajax({
                              ...
                              datatype.forEach(function (item, index) {
                                  //if (item.type ==1 || item.type ==3) {
                                    listlink.data[0].media.push({
                                      ...
                                      max_quality: item.type == 1 ?'文件夹' :item.name.substr(item.name.lastIndexOf('.') + 1).toUpperCase() ,
                ...
                error: function error() {
                  ...
                  _this.empty('访问Alist网站-' + object.title + '遇到问题。');
                }
              });
    ...
    Lampa.Template.add('settings_mod_alist', "<div>\n  <div class=\"settings-param selector\" data-name=\"alist_site_json\" data-type=\"input\" placeholder=\"\"> <div class=\"settings-param__name\">Json地址</div> <div class=\"settings-param__value\">请填写json地址</div> <div class=\"settings-param__descr\">填写Alist网站配置地址</div> </div>\n   </div>\n</div>");
    Lampa.Listener.follow('app', function (e) {
      ...
      menu_item.on('hover:enter', function () {
        if (catalogs){
          ...
          Lampa.Noty.show('Alsit配置无法加载，请在设置中检查JSON地址。');
      }
    }
    );
    ...
    Lampa.Settings.listener.follow('open', function (e) {
      if (e.name == 'mod_alist') {
        var item = e.body.find('[data-name="settings_mod_alist"]');
