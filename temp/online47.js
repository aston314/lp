          ...
          item.on('hover:enter', function () {
            ...
            if (element.file) {
              ...
              if (!!window.cordova) {
                ...
                var iabRef = cordova.InAppBrowser.open(proxy_url + element.file, "_blank", "location=no,hidden=yes,beforeload=yes,mediaPlaybackRequiresUserAction=no");
                iabRef.addEventListener('beforeload', function () {
                  iabRef.insertCSS({ code: 'body{background-color: black;'});
                });
                iabRef.addEventListener('loadstop', function () {
                  iabRef.insertCSS({ code: '.dplayer-web-fullscreen-fix1{background-color: black;position:fixed;top:0;left:0;margin:0;padding:0}.MacPlayer { position:fixed;z-index:2147483649;left:0;top:0;width:100%!important;height:100%!important}' });
                  ...
                  jQuery("div:not(.MacPlayer)").hide();  \
                  jQuery(".MacPlayer").appendTo("body"); \
                  '});
