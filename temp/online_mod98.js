...
(function () {
    ...
    var extract_rule = {
      "rule": [
        ...
        },
        {
          ...
          search_html_selector: '',
          link_folder: 'detail',
          detail_url_selector: '.stui-pannel__head:contains(LINELIB)+ul',
          use_referer: true,
          js_execute_key: ['maccms', 'player_aaaa']
        },
        {
    ...
    function web(component, _object, rule) {
      ...
      this.search = function (_object, kinopoisk_id) {
        ...
        network.silent(url1, function (str) {
          ...
          var searchresult = doreg.search_json ? parsedData[doreg.node_json].length : $(doreg.search_html_selector, parsedData).find('a').length;
      ...
      function getdetail(url) {
        ...
        if (window.cordovaHTTP) {
          ...
          //   return new Promise(function (resolve, reject) {
              ...
              cordovaHTTP.get(url, {}, {
                'User-Agent': 'lampa_client (Dalvik)',
    ...
    function zaihuapan(component, _object, rule) {
      var network = new Lampa.Reguest();
      var extract = {};
      var object = _object;
      var select_title = '';
      ...
      this.search = function (_object, kinopoisk_id) {
        ...
        select_title = object.movie.title;
        doreg = rule;
        var url1 = 'https://www.zaihuapan.com/?s=#msearchword';
        url1 = url1.replace('#msearchword',encodeURIComponent(object.movie.title));
      ...
      function parse(json) {
        ...
        var h = $('.post', str);
        $(h).each(function (i, html) {
          if ($('.post-title a', html).attr('title').match(/https:\/\/www\.aliyundrive\.com\/s\/([a-zA-Z\d]+)/)) {
            rslt.push({
              file: $('.post-title a', html).attr('title').match(/https:\/\/www\.aliyundrive\.com\/s\/([a-zA-Z\d]+)/)[0],
              quality: $('.post-time', html).text(),
              //quality: $('p',html).text().replace(/文件夹/,'目录'),
              title: $('.post-title a', html).text(),
              season: '',
              episode: '',
              info: ''
            });
    ...
    function Aliyundrive_Share_Channel(component, _object, rule) {
      var network = new Lampa.Reguest();
      var extract = {};
      var object = _object;
      var select_title = '';
      ...
      this.search = function (_object, kinopoisk_id) {
        ...
        select_title = object.movie.title;
        doreg = rule;
        var url1 = 'http://proxy.cub.watch/cdn/https://tx.me/s/Aliyundrive_Share_Channel?q=#msearchword';
        url1 = url1.replace('#msearchword',encodeURIComponent(object.movie.title));
      ...
      function transTimestamp(datetime, dateSeprator = '/', timeSeprator = ':') {
        if (datetime) {
        ...
        $(h).each(function (i, html) {
          ...
          if ($(html).text().match(/https:\/\/www\.aliyundrive\.com\/s\/([a-zA-Z\d]+)/)) {
            rslt.push({
              ...
              quality: '发布于：'+transTimestamp($('.time', html).attr('datetime'), "-", ":"),
              //quality: $('p',html).text().replace(/文件夹/,'目录'),
              title: $('.highlight:first', html).text()+$(html).html().match(/<\/mark>(.+?)<br>/)[1].replace(/<b>|<\/b>/g,'').replace(/<mark/g,'<span').replace(/mark>/g,'span>').replace('》',''),
              season: '',
              episode: '',
              info: ''
            });
    ...
    function component(object) {
      ...
      var sources = {
        ...
        在花盘: new zaihuapan(this, object),
        阿里云盘发布频道: new Aliyundrive_Share_Channel(this, object),
      };
      ...
      var filter_sources = ['酷云', '阿里云盘发布频道', '阿里云盘', '小纸条', '在花盘', 'videocdn', 'rezka', 'kinobase', 'collaps', 'cdnmovies', 'filmix', 'hdvb', 'videoapi']; // шаловливые ручки
