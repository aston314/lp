(function () {
    ...
    if (Lampa.Storage.get('json_use')) {
        new Lampa.Reguest()["native"](Lampa.Storage.field('web_rule_json')+'?v=' + Math.random(), function (json) {
            filtred_(json);
        }, function (a, c) {
        ...
        //catalogs = parseJSONSafely(getRemote(Lampa.Storage.field('web_rule_json')+'?v=' + Math.random()));
    } else {
        catalogs = [
            {
                title: "LIBVIO",
            ...
            if (typeof catalogs === 'undefined') {
                ...
                $(h).each(function (i, html) {
                    ...
                    switch(catalogs1[0].list.thumb.attrName) {
                        case 'text':
                          ii = i1.text().indexOf('http') == -1 ? host + i1.text() : i1.text();
                          break;
                        case 'html':
                          ii = i1.html();
                          break;
                        default:
                          ii = i1.attr(catalogs1[0].list.thumb.attrName) ? (i1.attr(catalogs1[0].list.thumb.attrName).indexOf('http') == -1 ? host + i1.attr(catalogs1[0].list.thumb.attrName) : i1.attr(catalogs1[0].list.thumb.attrName)) :'';     
                    };
