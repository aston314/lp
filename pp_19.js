(function () {
  ...
  function component(object) {
    ...
    var url, jsonsearch;
    var contextmenu_all = [];
    var proxy = Lampa.Storage.get('pikpak_proxy') ? url = 'https://cors.eu.org/': url = '';
    ...
    if (!Lampa.Storage.get("pikpakUserInfo","").loginInfo || Lampa.Storage.get("pikpakUserInfo","").loginInfo.expires < new Date().getTime()) {
      PikPakLogin(function () {
      }, function () {
      });
      if (Lampa.Storage.get("pikpakUserInfo","").loginInfo) {
        p = {
          dataType: "json",
          headers: {
            "content-type": "application/json;charset=utf-8",
            authorization: Lampa.Storage.get("pikpakUserInfo","").loginInfo.token_type + ' ' + Lampa.Storage.get("pikpakUserInfo","").loginInfo.access_token
          },
        };
      } else {
        p = {
