...
(function () {
    ...
    function noname(component, _object, rule) {
      ...
      this.search = function (_object, kinopoisk_id) {
        ...
        //console.log(select_title.replace(/第(.+)季/, '').trim());
        var url;
        kinopoisk_id === parseInt(kinopoisk_id, 10) ? url = cors_https + 'http://47.242.89.48/video/xiaoshu.php/detail?action=plus&code=8ff57a4df664974eca05970c7dadfc8e&vod_id=' + kinopoisk_id : url = cors_https + 'http://47.242.89.48/video/xiaoshu.php/list?action=plus&code=8ff57a4df664974eca05970c7dadfc8e&wd=' + encodeURIComponent(object.movie.title);
    ...
    function component(object) {
      ...
      var filter_sources = ['找资源', '小纸条', '猫狸盘搜', '易搜', '霸王龙压制组', '无名资源', '酷云',  'videocdn', 'rezka', 'kinobase', 'collaps', 'cdnmovies', 'filmix', 'hdvb', 'videoapi']; // шаловливые ручки
      ...
      this.start = function (first_select) {
        ...
        if (first_select) {
          ...
          if (data_filter.order === 1 && data_filter.last_viewed === '') {
            Lampa.Storage.set('online_mod_choice_first_order', true);
            Lampa.Storage.set('online_mod_choice_first_order_', 1);
          };

          if (typeof cont !== "undefined" && cont.order === 1 && cont.last_viewed >= 0) {
            Lampa.Storage.set('online_mod_choice_first_order_', 1);
          };

          if (typeof cont !== "undefined" && cont.order === 0 && cont.last_viewed >= 0) {
            Lampa.Storage.set('online_mod_choice_first_order_', 0);
          };
          
          var first_order = Lampa.Storage.get('online_mod_choice_first_order');
          //console.log(data_filter.order,data_filter.last_viewed,first_order,Lampa.Storage.get('online_mod_choice_first_order_'))
          
    	    var cont = data[selected_id || object.movie.id || object.movie.url];
          var neworder = cont && cont.last_viewed || 0 ;
          //console.log(cont,(typeof cont !== "undefined") ? (cont.order ? (cont.last_viewed >=0 ? (cont.order == temp ? neworder : - Math.abs(neworder + 1) ) : neworder): neworder) : neworder)
          //var new_order =  (typeof cont !== "undefined") ? (cont.order ? (cont.last_viewed >=0 ? (cont.order == Lampa.Storage.get('online_mod_choice_first_order_') ? neworder : (first_order === true ? neworder : - Math.abs(neworder + 1)) ) : neworder): neworder) : neworder;
          var new_order ;
          if (typeof cont !== "undefined") {
            if (Lampa.Storage.get('online_mod_choice_first_order_') === cont.order && cont.last_viewed >= 0) {
              if (first_order !== true) {
