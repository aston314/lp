(function () {
    ...
    function TG(object) {
        ...
        var waitload;
        var doubanitem = [];
        var cors = 'http://proxy.cub.watch/cdn/';
        ...
        this.card = function (str) {
            ...
            str = str.replace(/\n/g, '');
                ...
                page = $('.tme_messages_more.js-messages_more', str).attr('href').match(/\d+/);
                ...
                $('div.tgme_widget_message_wrap.js-widget_message_wrap',str).each(function (i, html) {  
                    ...
                    card.push({
                        ...
                        title: $('.tgme_widget_message_text.js-message_text', html).html().match(/(.+?)<br>/)[1].replace(/(<([^>]+)>)/ig, '').replace(/资源标题：|资源名称：|名称：/,''),
                        original_title: '',
                        title_org: '',
                        //url: catalogs1[0].list.link.attrName =='text' ? host+u1.text() : host+u1.attr(catalogs1[0].list.link.attrName),
                        url: $('.tgme_widget_message_text.js-message_text', html).text().match(/https:\/\/www\.aliyundrive\.com\/s\/([a-zA-Z\d]+)/)?$('.tgme_widget_message_text.js-message_text', html).text().match(/https:\/\/www\.aliyundrive\.com\/s\/([a-zA-Z\d]+)/)[0]:'',
    ...
    function startTG() {
        ...
        Lampa.Listener.follow('app', function (e) {
            if (e.type == 'ready') {
                var ico = '<svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" stroke="currentColor"><path d="M21 5 2 12.5l7 1M21 5l-2.5 15L9 13.5M21 5 9 13.5m0 0V19l3.249-3.277"  stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
