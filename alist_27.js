          ...
          item.on('hover:focus', function (e) {
            ...
            if (element.type == 1) {
              ...
              var file =element.file_id;
                ...
                if (file) {
                  //_this4.start();
                  if (file.indexOf('pdsapi') !== -1 && file.indexOf('redirect') !== -1) {
                    console.log('fffffuck')
                    //file = encodeURI(object.url.replace(object.url.match(/(http|https):\/\/(www.)?(\w+(\.)?)+/)[0], object.url.match(/(http|https):\/\/(www.)?(\w+(\.)?)+/)[0] + '/d') + '/' + element.translation)
                    $.ajax({
                      url: file,
                      type: 'GET',
                      async: false,
